...
trial_root="${TRIAL_ROOT:-$repo_root/.trial-opencode-go-subagent-$timestamp}"
memory_repo="$trial_root/agent-memory-source"
memory_worktree="$trial_root/agent-memory-worktree"
memory_branch="${MEMORY_BRANCH:-memory/opencode-go-deepseek-v4-flash-subagent-trial}"
...
harness["agent"]["condition_name"] = "opencode-go-subagent-memory-trial"
harness["agent"]["prompt_version"] = "2026-06-23-opencode-go-subagent"
...
harness["logging"]["harness_version"] = "local-opencode-go-subagent-trial"
...
(trial / "sts2harness.opencode-go-subagent.trial.json").write_text(
    json.dumps(harness, indent=2) + "\n",
    encoding="utf-8",
)

pi = json.loads(pathlib.Path("pi_agent/config/pi_agent.opencode-go.local.json").read_text())
pi["harness_config"] = str(trial / "sts2harness.opencode-go-subagent.trial.json")
pi["memory_root"] = str(worktree)
(trial / "pi_agent.opencode-go-subagent.trial.json").write_text(
...
orch["rpc_server_command"] = [
    ...
    "--config",
    str(trial / "pi_agent.opencode-go-subagent.trial.json"),
]
...
(trial / "orchestrator.opencode-go-subagent.trial.json").write_text(
...
echo "Starting OpenCode Go DeepSeek V4 Flash sub-agent trial run..."
...
python -m pi_orchestrator.orchestrator \
  --config "$trial_root/orchestrator.opencode-go-subagent.trial.json"

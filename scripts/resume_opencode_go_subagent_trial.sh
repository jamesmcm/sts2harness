...
if [[ $# -eq 1 ]]; then
  ...
  trial_root="$(ls -td "$repo_root"/.trial-opencode-go-subagent-* 2>/dev/null | head -n 1 || true)"
fi

if [[ -z "${trial_root:-}" ]]; then
  echo "No .trial-opencode-go-subagent-* directory found." >&2
  exit 1
fi
...
echo "Resuming OpenCode Go subagent trial: $trial_root"
